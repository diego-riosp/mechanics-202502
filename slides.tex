...
\begin{frame}{Sistema de partículas discreto}
...
\begin{frame}
    \begin{center}
        ...
        \large Dinámica del sólido rígido
    \end{center}
\end{frame}

\begin{frame}
    Un sólido rígido se define como aquel cuya distancia entre partículas que lo componen permanece invariante ante esfuerzos aplicados (objeto totalmente inelástico).
\end{frame}

\begin{frame}{Velocidad angular como un vector}

    $\omega_z$ es la componente $z$ del vector velocidad angular $\vec{\omega}$ dirigido a lo largo del eje de rotación. La dirección de $\vec{\omega}$ está dada por la regla de la mano derecha.
    
    \begin{figure}
        \centering
        \includegraphics[width=0.8\linewidth]{figures/omega-vector.png}
    \end{figure}
\end{frame}

\begin{frame}{Torque}
    El torque ejecutado sobre un sistema por una fuerza $\vec{F}$ en un punto cuya posición es $\vec{r}$ respecto del punto de giro del sistema, está <dado por la expresión \begin{equation*}
        \vec{\tau}=\vec{r}\times\vec{F}\,.
    \end{equation*}

    \begin{figure}
        \centering
        \includegraphics[width=0.5\linewidth]{figures/torque-example1.png}
    \end{figure}
\end{frame}

\begin{frame}{Ejemplo 1}
    Determine el torque neto en la situación ilustrada

    \begin{figure}
        \centering
        \includegraphics[width=0.5\linewidth]{figures/torque-neto1.png}
    \end{figure}
\end{frame}

\begin{frame}{Segunda ley de newton rotacional}
    El torque neto que actúa sobre un sistema es directamente proporcional a la aceleración angular del sistema. La constante de proporcionalidad es el momento de inercia del sistema \begin{equation*}
        \sum_{i=1}^N\vec{\tau}_i = I\vec{\alpha}\,.
    \end{equation*} \begin{center}
        \textit{¡El momento de inercia se presenta como una oposición al cambio de estado rotacional de un sistema!}
    \end{center}
\end{frame}

\begin{frame}{Ejemplo 1}
    Determine la aceleración angular dada la situación de la figura.

    \begin{figure}
        \centering
        \includegraphics[width=0.4\linewidth]{figures/ac-ang-y-torque.png}
    \end{figure}
\end{frame}

\begin{frame}{Ejemplo 2}
    La figura muestra un deslizador de masa $m_1$ que se mueve sin fricción sobre un riel horizontal de aire. Está sujeto a un objeto de masa $m_2$ con una cuerda de masa despreciable. La polea tiene radio $R$
y momento de inercia $I$ en torno a su eje de rotación. Cuando el objeto colgante se suelta,
acelera hacia abajo, el deslizador acelera a la derecha y la cuerda hace girar la polea sin
deslizarse ni estirarse. Ordene, de mayor a menor, las magnitudes de las siguientes fuerzas
que actúan durante el movimiento.

\begin{itemize}
    \item[\textit{i.}]La fuerza de tensión (magnitud $T_1$) en la parte
horizontal de la cuerda;
     \item[\textit{ii.}]la fuerza de tensión (magnitud $T_2$) en la parte vertical de
la cuerda;
      \item[\textit{iii.}]el peso $m_2g$ del objeto colgante.
\end{itemize}

\begin{figure}
    \centering
    \includegraphics[width=0.5\linewidth]{figures/ejemplo2-torque-acel.png}
\end{figure}

\end{frame}

\begin{frame}{Ejemplo 3}
    Un libro de texto de 2.00 kg descansa sobre una superficie horizontal sin fricción. Un cable unido al libro pasa sobre una polea, cuyo diámetro es de 0.150 m, para un libro con masa de 3.00 kg que cuelga. El sistema se libera a partir del reposo, y se observa que los libros se mueven 1.20 m en 0.800 s. \begin{itemize}
        \item[a)] Cuál es la tensión en cada parte de la cuerda?
        \item[b)] ¿Cuál es el momento de inercia de la polea alrededor de su eje de rotación?
    \end{itemize}
\end{frame}

\begin{frame}{Ejemplo 4: Equilibrio rotacional}
    Dada lo situación mostrada en la figura, determine el valor de la fuerza normale que aplica cada viga ($A$ y $B$) sobre el andamio.
    \begin{figure}
        \centering
        \includegraphics[width=0.8\linewidth]{figures/equilibrio-rot1.png}
    \end{figure}
\end{frame}

\begin{frame}
    \begin{center}
        \LARGE Rotación de un cuerpo\\ rígido \textbf{sobre un eje móvil}
    \end{center}
\end{frame}

\begin{frame}{Traslación y rotación combinadas}

\begin{figure}
    \centering
    \includegraphics[width=0.35\linewidth]{figures/rot-trasl-theo.png}
\end{figure}

    \begin{equation}
        K=\frac{1}{2}mv_{\mathrm{cm}}^2+\frac{1}{2}I_{\mathrm{cm}}\omega^2
    \end{equation}
\end{frame}

\begin{frame}{Rodar sin resbalar}
    \begin{figure}
        \centering
        \includegraphics[width=0.8\linewidth]{figures/rodar-sin-resbalar.png}
    \end{figure}

    \begin{center}
        ¡Un sistema uniforme de radio $R$ que rueda sin resbalar es equivalente a uno que gira alrededor de un eje que se encuentra a una distancia $R$ de su centro de masa!
    \end{center}
\end{frame}

\begin{frame}{La dinámica de rodar sin resbalar}

Para un yoyo cilíndrico de masa $M$ y radio $R$ que desciende una altura $h$, determine

\begin{itemize}
    \item[a)] La velocidad final de su centro de masa.
    \item[b)] La aceleración de su centro de masa.
    \item[c)] La tensión de la cuerda.
\end{itemize}

    \begin{figure}
        \centering
        \includegraphics[width=0.35\linewidth]{figures/yoyo.png}
    \end{figure}
\end{frame}

\begin{frame}{Otro ejemplo}

    Una bola de bolos sólida rueda sin resbalar bajando por una rampa
que está inclinada un ángulo $\beta$ con la horizontal. ¿Qué aceleración tiene la bola y cuál es la magnitud de la fuerza de fricción sobre esta? Trate la bola como esfera sólida uniforme, despreciando los agujeros.

    \begin{figure}
        \centering
        \includegraphics[width=0.5\linewidth]{figures/bola-de-bolos-rampa.png}
    \end{figure}
\end{frame}

\begin{frame}{Otro ejemplo}
    Una canica uniforme baja rodando por un tazón simétrico, partiendo del reposo en el borde izquierdo. El borde está una distancia $h$ arriba del fondo del tazón. La mitad izquierda del tazón es lo bastante áspera como para que la canica ruede sin resbalar, pero la mitad derecha no tiene fricción porque está lubricada con aceite.

    \begin{itemize}
        \item[a)] ¿Qué
altura alcanzará la canica en el lado resbaloso, medida verticalmente
desde el fondo?
\item[b)] ¿Qué altura alcanzaría la canica si el lado derecho
fuera tan áspero como el izquierdo?
\item[c)] ¿Cómo explica el hecho de que
la canica alcance más altura en el lado derecho con fricción que sin
fricción?
    \end{itemize}
\end{frame}

\begin{frame}{Ejemplo}
    Una bola de bolos sube rodando sin resbalar por una rampa que forma un ángulo $\beta$ con la horizontal. (Recordar ejemplo efectuado). Trate la bola como una esfera sólida uniforme, sin tomar en cuenta los agujeros.
    
    \begin{itemize}
        \item[a)] Dibuje
el diagrama de cuerpo libre de la bola.
Explique por qué la fricción debe
tener dirección cuesta arriba.
\item[b)] ¿Qué
aceleración tiene el centro de masa
de la bola?
\item[c)] ¿Qué coeficiente mínimo de fricción estática se necesita
para que la bola no resbale?
    \end{itemize}
\end{frame}

\begin{frame}{Trabajo efectuado por un torque}
    El trabajo que efectúa una fuerza que somete un sistema a una rotación diferencial $d\theta$, está dado por \begin{equation}
        dW=FR\,d\theta\,
    \end{equation} donde $\vec{F}$ es la magnitud de $\vec{F}$ y $R$ la magnitud del vector de posición hasta el punto de aplicación de la fuerza.

    Nótese que $\tau=FR$, por lo tanto, el trabajo neto está dado por \begin{equation*}
        W=\int_{\theta_1}^{\theta_2}\tau\,d\theta\,.
    \end{equation*}
\end{frame}

\begin{frame}{Teorema de las fuerzas vivas y potencia}
    Bajo esta definición, se cumple el teorema de las fuerzas vivas,

    \begin{equation*}
        W=\frac{1}{2}I\omega^2-\frac{1}{2}I\omega_0^2\,.
    \end{equation*}

    La potencia está dada por \begin{equation*}
        \mathcal{P}=\tau\omega\,.
    \end{equation*}
\end{frame}

\begin{frame}{Ejemplo}
    Un motor eléctrico consume 9.00 kJ de energía eléctrica en
1.00 min. Si un tercio de la energía se pierde en forma de calor y otras
formas de energía interna del motor, y el resto se da como potencia
al motor, ¿cuál será el valor de la torca que desarrollará este motor si
usted lo pone a 2500 rpm?
\end{frame}

\begin{frame}{Momento angular}
    El análogo del momento lineal de una partícula en el movimiento de rotación es el momento angular \begin{equation*}
        \vec{L}=\vec{r}\times\vec{p}\,.
    \end{equation*}

    \begin{figure}
        \centering
        \includegraphics[width=0.5\linewidth]{figures/L-angular.png}
    \end{figure}
\end{frame}

\begin{frame}{Relación torque-momento angular}
    Si el momento angular total del sistema de partículas es $\vec{L}$ y la suma de los
torques externos es $\sum \vec{\tau}$ entonces\begin{equation*}
    \sum_i\vec{\tau}_i=\frac{d\vec{L}}{dt}\,.
\end{equation*}
\end{frame}

\begin{frame}{Conservación del momento angular}
    \begin{center}
        \large \textit{Si el torque externo neto que actúa sobre un sistema es cero, el momento angular total del sistema es constante (se conserva).}
    \end{center}
    
\end{frame}

\begin{frame}{Ejemplo 1}
    Un bloque pequeño de 0.0250 kg en una superficie
horizontal sin fricción está atado
a una cuerda de masa despreciable que pasa por un agujero en
la superficie (ver figura). El
bloque inicialmente está girando
a una distancia de 0.300 m del
agujero, con rapidez angular de
1.75 rad$/$s. Ahora se tira de la
cuerda desde abajo, acortando el
radio del círculo que describe
el bloque a 0.150 m. El bloque puede tratarse como partícula. \begin{enumerate}
    \item[a)] ¿Se
conserva el momento angular del bloque? ¿Por qué?
    \item[b)] ¿Qué valor
tiene ahora la rapidez angular?
    \item[c)] Calcule el cambio de energía cinética
del bloque.
\item[d)] ¿Cuánto trabajo se efectuó al tirar de la cuerda?
\end{enumerate}

\begin{figure}
    \centering
    \includegraphics[width=0.3\linewidth]{figures/conserv-L1.png}
\end{figure}
\end{frame}

\begin{frame}{Ejemplo 2}
    Una varilla delgada uniforme tiene una longitud de 0.500 m
y está girando en un círculo sobre una mesa sin rozamiento. El eje de
rotación es perpendicular a la longitud de la varilla en un extremo y
está en reposo. La varilla tiene una velocidad angular de 0.400 rad$/$s
y un momento de inercia sobre el eje de $3.00 \times 10^{-3}$ $\text{kg}\cdot \text{m}^2$. Un bicho,
inicialmente parado en el extremo de la varilla en el eje de rotación,
decide arrastrarse hasta el otro extremo de la varilla. Cuando el bicho
ha alcanzado el extremo de la varilla y se sienta allí, su rapidez tangencial es 0.160 m$/$s. El bicho se puede tratar como una masa puntual.
\begin{enumerate}
    \item[a)] ¿Cuál es la masa de la barra?
    \item[b)] ¿Cuál es la masa del bicho?
\end{enumerate}
\end{frame}

\begin{frame}{Ejemplo 3}
    Un proyectil de masa m se mueve hacia la derecha con una rapidez $v_0$
(ver figura). El proyectil golpea y se pega al
extremo de una barra estable de masa $M$ y longitud $d$ articulada en torno a un eje sin fricción a través de su centro. 

\begin{enumerate}
    \item[a)] Encuentre la rapidez angular del sistema justo
después de la colisión.
    \item[b)] Determine la pérdida fraccional en
energía mecánica debido a la colisión.
\end{enumerate}

\begin{figure}
    \centering
    \includegraphics[width=0.5\linewidth]{figures/barra-L.png}
\end{figure}
\end{frame}

\begin{frame}{Ejemplo 4}
    Un cilindro con momento de inercia $I_1$ da vueltas en torno a
un eje vertical sin fricción con rapidez angular $\omega_0$. Un segundo
cilindro, con momento de inercia $I_2$ y que inicialmente no
gira, cae sobre el primer cilindro (ver figura). Debido a
la fricción entre las superficies, con el tiempo los dos llegan a la
misma rapidez angular $\omega$.

\begin{enumerate}
    \item[a)] Calcule $\omega$.
    \item[b)] Demuestre que la
energía cinética del sistema disminuye en esta interacción y
calcule la proporción de la energía rotacional final a la inicial.
\end{enumerate}

\begin{figure}
    \centering
    \includegraphics[width=0.5\linewidth]{figures/perdida-energia.png}
\end{figure}

\end{frame}

\begin{frame}{El giroscopio}
    \begin{columns}
        \column{0.45\textwidth}
        \begin{figure}
            \centering
            \includegraphics[width=0.9\linewidth]{figures/volante2.png}
        \end{figure}
        \column{0.45\textwidth}
        \begin{figure}
            \centering
            \includegraphics[width=0.9\linewidth]{figures/volante3.png}
        \end{figure}
    \end{columns}
\end{frame}


\begin{frame}{Cómo cambia $\vec{L}$}
    \begin{figure}
        \centering
        \includegraphics[width=0.95\linewidth]{figures/volante.png}
    \end{figure}
\end{frame}

\begin{frame}{Velocidad angular de precesión}
    \begin{figure}
        \centering
        \includegraphics[width=0.45\linewidth]{figures/giroscopio.png}
    \end{figure}

    \begin{equation}
        \sin(d\phi)\approx d\phi = \frac{dL}{L}=\frac{\tau\,dt}{L}=\frac{(Mgh)\,dt}{L}\,.
    \end{equation} De esta ecuación, se deduce que \begin{equation}
        \omega_p=\frac{Mgh}{I\omega}
    \end{equation}
    \begin{center}
        \textit{La rapidez angular $\omega_p$ se llama frecuencia de precesión.}
    \end{center}
\end{frame}


\begin{frame}{El trompo}
\begin{center}
    ¡Un trompo funciona exactamente igual!
\end{center}
    \begin{figure}
        \centering
        \includegraphics[width=0.35\linewidth]{figures/trompo.png}
    \end{figure}
...
\begin{frame}
    \begin{center}
    \LARGE
        \textit{El trompo \textbf{NO} cae al girar puesto que el momento angular no nulo garantiza que el torque gravitacional generará incrementos de momento angular en el plano de precesión y no un giro efectivo hacia el suelo}
    \end{center}
\end{frame}

\begin{frame}{Ejemplo}
    El rotor (volante) de un giróscopo de juguete tiene una masa de
0.140 kg. Su momento de inercia alrededor de su eje es $1.20 \times 10^{-4} \mathrm{kg}\cdot\mathrm{m}^2$.
La masa del marco es de 0.0250 kg. El giróscopo se apoya en un solo
pivote (ver figura) con su centro de masa a una distancia horizontal
de 4.00 cm del pivote. El giróscopo hace un movimiento de precesión
en un plano horizontal a razón de una revolución cada 2.20 s. \begin{enumerate}
    \item[a)] Calcule la fuerza hacia arriba ejercida por el pivote.
    \item[b)] Calcule la rapidez
angular en rpm con que el rotor gira sobre su eje.
\item[c)] Copie el diagrama
e indique con vectores el momento angular del rotor y la torca que
actúa sobre este.
\end{enumerate}
\begin{figure}[H]
    \centering
    \includegraphics[width=0.3\linewidth]{figures/giroscopio-ejmplo.png}
\end{figure}
\end{frame}

\begin{frame}{Ejercicio bonus: Estática rotacional}
En un zoológico, una varilla uniforme de 240 N y 3.00 m
de longitud se sostiene en posición horizontal con dos cuerdas en sus
extremos (ver figura). La cuerda izquierda forma un ángulo de $150^\circ$ con la varilla, y la derecha forma un ángulo $\theta$ con la horizontal. Un mono aullador (\textit{Alouatta seniculus}) de 90 N cuelga inmóvil a 0.50 m del extremo derecho de la varilla y nos estudia detenidamente.
Calcule $\theta$ y las tensiones en las dos cuerdas. Comience dibujando un
diagrama de cuerpo libre de la varilla.
    \begin{figure}
        \centering
        \includegraphics[width=0.5\linewidth]{figures/estatica-rot.png}
    \end{figure}
\end{frame}